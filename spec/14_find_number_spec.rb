...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 5 and max is 9' do
      subject(:game_guessing_five_and_nine) { described_class.new(5, 9, number_guessing) }
      it 'returns 7' do
        expect(game_guessing_five_and_nine.make_guess).to eql(7)
      end
    end

    context 'when min is 8 and max is 9' do
      subject(:game_guessing_eight_and_nine) { described_class.new(8, 9, number_guessing) }
      it 'returns 8' do
        expect(game_guessing_eight_and_nine.make_guess).to eql(8)
      end
    end

    context 'when min is 0 and max is 3' do
      subject(:game_guessing_zero_and_three) { described_class.new(0, 3, number_guessing) }
      it 'returns 1' do
        expect(game_guessing_zero_and_three.make_guess).to eql(1)
      end
    end

    context 'when min and max both equal 3' do
      subject(:game_guessing_three_and_three) { described_class.new(3, 3, number_guessing) }
      it 'returns 3' do
        expect(game_guessing_three_and_three.make_guess).to eql(3)
      end
    end
  end
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      subject(:game_guessing) do
        described_class.new(0, 9, random_number, random_number.value)
      end
    ...
    context 'when guess and random_number are not equal' do
      let(:random_number) { double('random_number', value: 5) }
      subject(:game_guessing) { described_class.new(0, 9, random_number, 3) }
  ...
  describe '#update_range' do
    ...
    context 'when the guess is less than the answer' do
      ...
      it 'updates min to 5' do
        current_guess = low_guess_game.guess
        expect do
          low_guess_game.update_range
        end.to change { low_guess_game.min }.by(current_guess + 1)
      end
    ...
    context 'when the guess is more than the answer' do
      ...
      it 'does not update min' do
        ...
        end.not_to(change { high_guess_game.min })
      end

      it 'updates max to 8' do
        current_guess = high_guess_game.guess
        expected_value = current_guess - 1
        result = high_guess_game.update_range
    ...
    context 'when the guess is 7, min=5, and max=8' do
      ...
      it 'updates min to the same value as max' do
        expected_difference = edge_guess_game.max - edge_guess_game.min

        expect do
          edge_guess_game.update_range
        end.to change(edge_guess_game, :min).by(expected_difference)

        expect(edge_guess_game.min).to eql(edge_guess_game.max)
      end

      it 'does not update max' do
        expect do
          edge_guess_game.update_range
        end.not_to(change { edge_guess_game.max })
      end
    end
  end
end
